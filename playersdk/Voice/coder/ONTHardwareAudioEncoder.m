...
@interface ONTHardwareAudioEncoder () {
...
@property (nonatomic, strong) ONTLiveAudioConfiguration *configuration;
@property (nonatomic, weak) id<ONTAudioEncodingDelegate> aacDeleage;

@end

@implementation ONTHardwareAudioEncoder

- (instancetype)initWithAudioStreamConfiguration:(nullable ONTLiveAudioConfiguration *)configuration {
    if (self = [super init]) {
...
- (void)setDelegate:(id<ONTAudioEncodingDelegate>)delegate {
...
- (void)encodeBuffer:(char*)buf timeStamp:(uint64_t)timeStamp{
    ...
    ONTAudioFrame *audioFrame = [ONTAudioFrame new];
...
//    audioFrame.data = [NSData dataWithBytes:aacBuf length:outBufferList.mBuffers[0].mDataByteSize];
    ...
    NSData *atdsData = [self adtsData:_configuration.numberOfChannels rawDataLength:data.length];
    NSMutableData *audioData = [NSMutableData dataWithData:atdsData];
