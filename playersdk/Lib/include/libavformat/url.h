...
typedef struct URLContext {
    ...
    int is_connected;
    AVIOInterruptCB interrupt_callback;       // 只读 或者 只写时使用
    AVIOInterruptCB reverse_interrupt_callback; // 读的时候同时写 或者 写的时候同时读时使用
    int64_t rw_timeout;         /**< maximum time to wait for (network) read/write operation completion, in mcs */
    const char *protocol_whitelist;
    const char *protocol_blacklist;
    int min_packet_size;        /**< if non zero, the stream is packetized with this min packet size */
    int max_report_size;
    int write_duplex_flag;    /** 0 or 1*/
    AVIODuplexListener duplex_listener;
} URLContext;
...
typedef struct URLProtocol {
    ...
    int     (*url_write_duplex)(URLContext *h, const unsigned char pkt_type, const unsigned char *buf, int size);
...
int ffurl_alloc_duplex(URLContext **puc, const char *filename, int flags,
                       const AVIOInterruptCB *int_cb, const AVIODuplexListener *duplex_listener);
/**
...
int ffurl_open_whitelist(URLContext **puc, const char *filename, int flags,
               ...
               URLContext *parent);
int ffurl_open_whitelist_duplex(URLContext **puc, const char *filename, int flags,
                                const AVIOInterruptCB *int_cb, const AVIODuplexListener *duplex_listener, AVDictionary **options,
                                const char *whitelist, const char* blacklist,
                                URLContext *parent);
...
int ffurl_write_duplex(URLContext *h, unsigned char pkt_type, const unsigned char *buf, int size,
                       int ts);
/**
